...
const recentSessions = [
  { date: '2025-11-25', topic: 'Meditation on Inner Peace' },
  { date: '2025-11-20', topic: 'Mantra for Happiness' }
];
  ...
  return (
    <div className="relative min-h-screen flex flex-col items-center justify-start px-4 pt-4 pb-10 overflow-hidden">
      ...
      <button
        ...
        className="relative px-12 py-4 rounded-full bg-gold text-black font-semibold text-lg md:text-xl
                   hover:bg-yellow-400 hover:scale-105 hover:shadow-xl transition transform duration-300 ease-in-out
                   before:absolute before:inset-0 before:rounded-full before:bg-gradient-to-r before:from-yellow-300 before:via-yellow-400 before:to-yellow-300
                   before:opacity-50 before:blur-xl before:animate-pulse before:-z-10 mb-16"
      >
        Continue
      </button>
      ...
      <section className="w-full max-w-5xl mb-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          {features.map((feature, idx) => (
            <div key={idx} className="p-5 rounded-xl bg-cardDark shadow-md hover:shadow-xl transition cursor-pointer">
      ...
      {recentSessions.length > 0 && (
        <section className="w-full max-w-4xl mb-10">
          <h2 className="text-xl font-semibold text-textLight mb-4 text-center">Your Recent Sessions</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
            {recentSessions.map((session, idx) => (
              <div key={idx} className="p-4 rounded-xl border bg-cardDark hover:bg-cardDark/90 transition cursor-pointer">
                <p className="font-semibold text-textLight">{session.topic}</p>
